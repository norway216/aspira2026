...
func (s *RiskService) AssessPayment(req payment.CreateRequest) (*risk.RiskResult, error) {
	...
	// Store request data for closure use
...
// Temporary fields for AssessPayment closure use
var (
	senderUserID   string
...
func (s *RiskService) checkKYCCompleted() (bool, string, int) {
	u, err := s.db.GetUserByID(senderUserID)
	if err != nil || u.Status != user.UserActive {
		return true, "Sender has not completed KYC", 100
	}

	kycProfile, err := s.db.GetKYCProfile(senderUserID)
...
func (s *RiskService) checkUserFrozen() (bool, string, int) {
	...
	receiver, err := s.db.GetUserByID(receiverUserID)
	if err != nil {
...
func (s *RiskService) checkAmountLimit() (bool, string, int) {
	u, _ := s.db.GetUserByID(senderUserID)
	riskLevel := "LOW"
	if u != nil {
	...
	if sourceAmount > limits.SingleTxLimit {
...
func (s *RiskService) checkDailyLimit() (bool, string, int) {
	dailyTotal, err := s.db.GetDailyTotal(senderUserID)
	if err != nil {
	...
	if dailyTotal+sourceAmount > limits.DailyLimit {
...
func (s *RiskService) checkRestrictedCountry() (bool, string, int) {
	...
	if restricted[countryFrom] {
		return true, "Source country is restricted", 100
	}
	if restricted[countryTo] {
...
func (s *RiskService) checkHighFrequency() (bool, string, int) {
	count, err := s.db.GetRecentTxCount(senderUserID, 60)
	if err != nil {
		return false, "", 0
	}
	if count >= 10 {
...
func (s *RiskService) checkNewUserLargeAmount() (bool, string, int) {
	...
	hoursSinceCreation := 0.0 // Simplified — in production, compute actual hours
	_ = hoursSinceCreation

	if u.CreatedAt.Unix() > 0 && sourceAmount > 100000 { // $1,000
...
func (s *RiskService) checkSanctionedCountry() (bool, string, int) {
	...
	restricted := risk.RestrictedCountries()
	if restricted[countryFrom] || restricted[countryTo] {
