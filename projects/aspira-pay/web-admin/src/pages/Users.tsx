        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 mb-6">
          ...
          <form onSubmit={handleRegister} className="space-y-4 max-w-md">
            ...
            <div className="flex gap-3">
              <button type="submit" className="px-6 py-2 bg-blue-600 hover:bg-blue-700 rounded-lg text-sm">Register</button>
              <button type="button" onClick={() => setShowRegister(false)} className="px-6 py-2 bg-gray-700 rounded-lg text-sm">Cancel</button>
            </div>
          </form>
        </div>
      )}
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-800 text-left">
                <th className="p-4 text-sm text-gray-500">User ID</th>
                <th className="p-4 text-sm text-gray-500">Username</th>
                <th className="p-4 text-sm text-gray-500">Email</th>
                <th className="p-4 text-sm text-gray-500">Status</th>
                <th className="p-4 text-sm text-gray-500">Risk Level</th>
              </tr>
            </thead>
            <tbody>
              {users.length === 0 ? (
                <tr><td colSpan={5} className="p-8 text-center text-gray-600">No users registered</td></tr>
              ) : users.map((u: any) => (
                <tr key={u.user_id} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                  <td className="p-4 text-sm font-mono text-gray-300">{u.user_id}</td>
                  <td className="p-4 text-sm">{u.username}</td>
                  <td className="p-4 text-sm text-gray-400">{u.email}</td>
                  <td className="p-4 text-sm">{u.status}</td>
                  <td className="p-4 text-sm">{u.risk_level}</td>
                </tr>
              ))}
            </tbody>
          </table>
