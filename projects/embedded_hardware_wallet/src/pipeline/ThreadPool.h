 ...
 *
 * Features:
 * - Priority queue: HIGH (signing), NORMAL (verification), LOW (backup/network)
 * - Work-stealing between queues for load balancing
 * - std::jthread with stop_token support
 * - Configurable thread count based on hardware concurrency
 */
class ThreadPool {
public:
